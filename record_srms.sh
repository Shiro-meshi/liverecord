		...
		if [[ $LIVE_STATUS_YOUTUBE -lt 1 ]] || [[ $EXCEPT == "except" ]]; then
			...
			if (echo $LOCAL_PAGE | grep "ytplayer" | grep -q '\"isLive\":true'); then
				...
				[[ $EXCEPT != "except" ]] && let LIVE_STATUS_YOUTUBE-- && let LIVE_STATUS_YOUTUBE_BEFORE--
                        ...
                        STREAMID=$(echo $LOCAL_PAGE | grep -o '\"liveStreamabilityRenderer\":{\"videoId\":\".*\"' | head -n 1 | awk -F'"' '{print $6}')
			if [[ $STREAMID == "ef2n23E8S8s" ]]; then
				STATUS=0
			fi
		fi
	fi
...
function prasepage(){
	...
	if [[ $TYPE == "twitcastffmpeg" ]]; then
		STREAM_URL=$(wget -q -O- "https://twitcasting.tv/${PART_URL}/metastream.m3u8?mode=source" | grep ".m3u8" | head -n 1)
	fi
...
function up(){
	...
	if [[ ! -f "${DIR}/${FNAME}" ]] || [[ $(ls -l "${DIR}/${FNAME}" | awk '{print $5}') == 0 ]]; then #判断是否无录像
		...
		ffmpeg -i "${DIR}/${FNAME}" -vcodec copy -acodec copy "${DIR}/${FNAME}_trans.mp4" > "${DIR}/${FNAME}_trans.log" 2>&1
		if [[ -n ${!UPLOAD_TYPE_LIST[*]} ]]; then #存在上传列表才上传
			upload "${DIR}/${FNAME}_trans.mp4"
			upload "${DIR}/${FNAME}.log"
			upload "${DIR}/${FNAME}_trans.log"
		fi
	fi
}
